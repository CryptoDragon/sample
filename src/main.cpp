...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // samplecoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "samplecoin Signed Message:\n";
...
{
    // samplecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // samplecoin: 1 day
static const int64 nTargetSpacing = 60; // samplecoin: 1 minute
...
{
        static const int64 BlocksTargetSpacing = 60; // samplecoin: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // samplecoin: 1 day
...
{
    // samplecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // samplecoin: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("samplecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("samplecoinMiner : ProcessBlock, block not accepted");
    }
